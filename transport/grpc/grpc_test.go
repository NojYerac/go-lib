...
var _ = Describe("grpc", func() {
	...
	BeforeEach(func() {
		...
		clientConn, err := ClientConn("localhost", WithDialOptions(testOpts...), WithHealthChecker(hc))
		Expect(err).NotTo(HaveOccurred())
...
func (*server) BidirectionalStreamingEcho(srv pb.Echo_BidirectionalStreamingEchoServer) error {
	logger := log.FromContext(srv.Context())
	for {
		req, err := srv.Recv()
		if err != nil {
			if err == io.EOF {
			...
			logger.WithError(err).Error("failed to receive streaming echo message")
			return err
		}
		msg := req.Message
		if msg == triggerPanic {
		...
		if err := srv.Send(res); err != nil {
			logger.WithError(err).Error("failed to send streaming echo message")
			return err
		}
	}
}
